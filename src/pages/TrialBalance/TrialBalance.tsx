...
export function TrialBalance() {
  ...
  const handleGenerate = () => {
    ...
    setLabel(currentPeriod.name)
    generate(currentPeriod.id, currentPeriod.start_date, currentPeriod.end_date)
  }
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            <div className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm bg-slate-50 text-slate-700">
              {currentPeriod ? `${currentPeriod.name} (${currentPeriod.start_date} — ${currentPeriod.end_date})` : 'No period selected'}
            </div>
          </div>
          {currentPeriod && (
