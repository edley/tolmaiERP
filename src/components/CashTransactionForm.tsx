...
export function CashTransactionForm({ onClose, onSuccess, payment }: CashTransactionFormProps) {
  ...
  const addLine = () => {
    if (readonly) return
    setLines([...lines, { id: nextLineId.current++, gl_account_id: '', amount: '' }])
    ...
    <form onSubmit={handleSubmit} className="flex flex-col h-full gap-0 max-w-7xl mx-auto w-full">
      ...
      <div className="flex-1 flex flex-col min-h-0">
        <div className="flex-1 flex flex-col min-h-0 border border-b-0 border-[#dddbda] rounded-t-lg bg-white shadow-sm">
          ...
          <div className="hidden lg:grid grid-cols-[40px_1fr_60px_140px_120px_80px] gap-0 px-4 py-2 bg-white border-b border-[#dddbda] text-[11px] font-bold text-[#514f4d] uppercase tracking-wider shrink-0">
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto min-h-0">
            ...
            {linesWithRunning.map((line, idx) => (
              <div
                key={line.id}
                className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[40px_1fr_60px_140px_120px_80px] gap-2 sm:gap-2 lg:gap-0 px-2 sm:px-3 lg:px-4 py-2 border-b border-[#dddbda] last:border-b-0 hover:bg-[#fafaf9] transition-colors lg:border-l-2 lg:border-l-red-400"
              >
      ...
      {/* ── AUDIT TRAIL ── */}
      {payment && <div className="mt-4"><AuditTrail data={payment} /></div>}
    </form>
  )
}
