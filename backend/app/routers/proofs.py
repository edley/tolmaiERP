        ...
        "items": [
            {
                ...
                "erp_status": p.erp_status.value if p.erp_status else None,
...
def get_proof(proof_id: str, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "source": proof.source,
        "status": proof.status.value if proof.status else None,
        "erp_status": proof.erp_status.value if proof.erp_status else None,
